...
Figures 1 and 2 displayed in previous pages show a similar trend as that in the paper, with additional years past 2013. In Figure 6, we can see the graph on share prices and theoretical values. It is relatively similar to the one in the paper [@kaizoji], given a few minor differences. FOr example, our plot has greater deviation especially towards the tail-end on the left-hand side. However, both plots have a similar general trend and both have a cluster of outliers in similar areas.

CAN ADD THE OTHER GRAPHS TOO BUT HAVEN'T GIVEN SPACE LIMIT.

```{r figs, out.width = "400px", echo=FALSE, fig.cap="\\label{fig:figs} Share Price and Theoretical Value"}
knitr::include_graphics("true_vs_predicted.jpg")
```
